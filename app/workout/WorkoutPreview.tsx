...
function ExerciseDetailModal({
      ...
      PanResponder.create({
        ...
        onPanResponderRelease: (_, gs) => {
          if (gs.dy > SWIPE_THRESHOLD || gs.vy > 0.5) {
            Animated.parallel([
              Animated.timing(translateY, { toValue: screenHeight, duration: 250, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
              Animated.timing(backdropOpacity, { toValue: 0, duration: 250, useNativeDriver: true }),
            ]).start(() => onClose());
          } else {
            Animated.parallel([
              Animated.spring(translateY, { toValue: 0, damping: 20, mass: 0.8, stiffness: 200, useNativeDriver: true }),
  ...
  useEffect(() => {
    if (visible) {
      Animated.parallel([
        Animated.spring(translateY, { toValue: 0, damping: 22, mass: 0.9, stiffness: 200, useNativeDriver: true }),
  ...
  return (
    <View style={[StyleSheet.absoluteFill, { zIndex: 100 }]} pointerEvents={visible ? "auto" : "none"}>
      <Animated.View style={[StyleSheet.absoluteFill, { backgroundColor: isDark ? "rgba(0,0,0,0.80)" : "rgba(0,0,0,0.50)", opacity: backdropOpacity }]}>
      ...
      >
        <View style={{ alignSelf: "center", width: 40, height: 4, borderRadius: 999, backgroundColor: isDark ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.12)", marginTop: 10, marginBottom: 8 }} />

        <Pressable
          onPress={handleClose}
          style={{ position: "absolute", top: 10, right: 16, zIndex: 10, width: 34, height: 34, borderRadius: 17, backgroundColor: SOFT, borderWidth: StyleSheet.hairlineWidth, borderColor: BORDER, alignItems: "center", justifyContent: "center" }}
          hitSlop={8}
        >
        ...
        <ScrollView
          ...
          contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 14, paddingBottom: Math.max(insets.bottom, 24) + 24 }}
          bounces
        >
          <Text style={{ fontSize: 24, fontWeight: "900", color: colors.text, letterSpacing: -0.35, lineHeight: 30, paddingRight: 44 }} numberOfLines={3}>
            {exercise.name}
          </Text>
          <Text style={{ marginTop: 6, fontSize: 14, fontWeight: "700", color: colors.muted }}>
            {exercise.sets.length} sets · Tempo {exercise.tempo} · {exercise.unitLabel}
          </Text>
          ...
          <View style={{ marginTop: 18 }}>
            {hasVideo ? (
              <Pressable
                ...
                style={{ borderRadius: 16, overflow: "hidden", marginBottom: 18, backgroundColor: "#000", height: 210 }}
              >
                ...
                <View style={{ ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.35)", alignItems: "center", justifyContent: "center" }}>
                  <View style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: "rgba(255,255,255,0.95)", alignItems: "center", justifyContent: "center" }}>
                    <View style={{ width: 0, height: 0, marginLeft: 3, borderLeftWidth: 18, borderTopWidth: 11, borderBottomWidth: 11, borderLeftColor: "#111", borderTopColor: "transparent", borderBottomColor: "transparent" }} />
                  </View>
                  <Text style={{ marginTop: 10, fontSize: 13, fontWeight: "800", color: "#fff" }}>Watch tutorial</Text>
                </View>
              </Pressable>
            ) : (
              <Image source={{ uri: exercise.image }} style={{ width: "100%", height: 210, borderRadius: 16, marginBottom: 18, backgroundColor: SOFT }} resizeMode="cover" />
            )}
          </View>

          {hasDescription && (
            <View style={{ marginBottom: 20 }}>
              <Text style={{ fontSize: 12, fontWeight: "900", color: colors.muted, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>About</Text>
              <Text style={{ fontSize: 16, lineHeight: 24, fontWeight: "600", color: colors.text }}>{exercise.description}</Text>
            </View>
          )}

          {hasTutorial && (
            <View style={{ marginBottom: 20 }}>
              <Text style={{ fontSize: 12, fontWeight: "900", color: colors.muted, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 12 }}>How to perform</Text>
              {exercise.tutorial!.map((step, i) => (
                <View key={i} style={{ flexDirection: "row", alignItems: "flex-start", gap: 12, marginBottom: i < exercise.tutorial!.length - 1 ? 14 : 0 }}>
                  <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: colors.text, alignItems: "center", justifyContent: "center", marginTop: 1 }}>
                    <Text style={{ fontSize: 12, fontWeight: "900", color: colors.surface }}>{i + 1}</Text>
                  </View>
                  <Text style={{ flex: 1, fontSize: 15, lineHeight: 22, fontWeight: "600", color: colors.text }}>{step}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {hasMuscles && (
            <View>
              <Text style={{ fontSize: 12, fontWeight: "900", color: colors.muted, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 12 }}>Muscles worked</Text>
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {exercise.musclesWorked!.map((m) => (
                  <View key={m} style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: SOFT, borderWidth: StyleSheet.hairlineWidth, borderColor: BORDER }}>
...
export function WorkoutPreview({
  ...
  const railColor = (block: StrengthBlock) => {
    ...
    if (block.type === "circuit") return isDark ? "rgba(255,255,255,0.40)" : "rgba(0,0,0,0.45)";
      ...
      >
        ...
        <View style={[styles.header, { paddingTop: Math.max(insets.top, 20) + 4 }]}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.headerTitle} numberOfLines={2}>{workoutTitle}</Text>
          ...
          {blocks.map((block) => {
            ...
            return (
              <View key={block.id} style={styles.blockWrap}>
                {/* Vertical rail — absolute, no radius, matches player */}
...
function createStyles(
  colors: {
    background: string; surface: string; card: string; text: string;
    muted: string; border: string; borderSubtle: string; premium: string;
  },
  isDark: boolean,
) {
  const BORDER = colors.borderSubtle ?? colors.border;
  ...
  return StyleSheet.create({
    ...
    // ── Block card — matches player groupWrap style ──────────
    blockWrap: {
      position: "relative",
      paddingLeft: 14,
    },
